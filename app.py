...
# We let DNS/TLS/SNI proceed completely normally (so legitimate hosts like
# www.iana.org and example.com behave exactly as they would un-intercepted),
# then inspect the ACTUAL peer IP the socket connects to, the instant after
# connection is established. If it's private/special, we abort immediately.
# This closes the DNS-rebinding TOCTOU race (validated hostname resolves
# publicly at check-time, but a second lookup inside the HTTP client returns
# a private IP at connect-time) without touching hostname resolution, TLS,
# or headers -- so it cannot cause any benign-side regressions.
...
def do_read_file(raw_path: str) -> Dict[str, Any]:
    if not raw_path:
        return {"action": "block", "reason": "Missing path."}
